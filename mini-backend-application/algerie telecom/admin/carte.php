...
$sql_adsl = "SELECT * FROM carte,produits WHERE carte.idproduit = produits.id AND produits.nature = 'ADSL'";
$racords_adsl = mysqli_query($db, $sql_adsl);
if($racords_adsl)
{
    while($row_adsl = mysqli_fetch_array($racords_adsl))
    {
        $data_adsl[] = $row_adsl;   
    }

    $sql_4glte = "SELECT * FROM carte,produits WHERE carte.idproduit = produits.id AND produits.nature = '4GLTE'";
    $racords_4glte = mysqli_query($db, $sql_4glte);
    if($racords_4glte)
    {
        while($row_4glte = mysqli_fetch_array($racords_4glte))
        {
            $data_4glte[] = $row_4glte;   
        }
    }else{
        //error
    }
            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-light" aria-label="">
             ...
             <div class="row justify-content-center">
                <div class="col-xs-11 col-md-5 mx-2 mb-3 px-4 py-3 cards card-1 d-flex flex-row justify-content-between align-items-center">
                    <div>
                        <span><?= $data_staistique[0]['revenueADSL_carte']?> <span>DA</span></span>
                        <div>total Bénéfices</div>
                    </div>
                    <div class="">
                ...
                <div class="col-xs-11 col-md-5 mx-2 mb-3 px-4 py-3 cards card-3 d-flex flex-row justify-content-between align-items-center">
                    <div>
                        <span><?= $data_staistique[0]['revenue4G_carte']?> <span>DA</span></span>
                        <div>total Bénéfices</div>
                    </div>
                    <div class="">
            ...
            <div class="list-client">
                <div class="col-md-12">
                    <div class="content-panel">
                        <?php 
                            echo "<h4 style='display: inline-block; color: #7289da;'><i class='fa fa-angle-right'></i> ADSL Carte</h4>";
                            echo "<a href='./ajouter-carte.php?nature=ADSL' class='btn btn-success btn-md' style='float: right; margin-right: 10px;' title='ajouter carte ADSL'>";
                        ...
                        <div id="scrollContent">
                            <table class="table table-striped table-advance table-hover">
                                ...
                                <tbody id="scrollContent">
                                    <?php 
                                    foreach($data_adsl as $carteData)
                                    {
                                       echo "<tr>";
                                       echo "<th scope='row'>".$carteData["id"]."</th>";
                                       echo "<td>".$carteData["nom"]."</td>";
                                       echo "<td>".$carteData["n_serie"]."</td>";
                                       echo "<td>".$carteData["code"]."</td>";
                                       echo "<th scope='col'>".$carteData["prix"]."</th>";
                                       echo "<td style='float: right;width:135px;' class='d-flex flex-row justify-content-between'>";
                                       echo "<a href='./detail-carte.php?n_serie=".$carteData["n_serie"]."' class='btn btn-success btn-sm' title='plus détail'><i class='fa fa-info-circle'></i></a>";
                                       echo "<a href='./modifier-carte.php?n_serie=".$carteData["n_serie"]."' class='btn btn-primary btn-sm' title='modifier information client'><i class='fa fa-pencil-alt'></i></a>";
                                       if($carteData["status"] != "Active"){
                                        echo "<form action='delete.php' method='POST'><button type='submit' name='DELETE_carte' value='".$carteData["n_serie"]."' class='btn btn-danger btn-sm' title='supprimer client' disabled><i class='fa fa-trash-alt'></i></button></from></td></tr>";
                                       }else{
                                        echo "<form action='delete.php' method='POST'><button type='submit' name='DELETE_carte' value='".$carteData["n_serie"]."' class='btn btn-danger btn-sm' title='supprimer client' ><i class='fa fa-trash-alt'></i></button></from></td></tr>";
                                       }
                                        echo "</td></tr>";
                                    }
                                    ?>
             ...
             <div class="list-client">
                <div class="col-md-12">
                    <div class="content-panel">
                        <?php 
                            echo "<h4 style='display: inline-block; color: #7289da;'><i class='fa fa-angle-right'></i> 4GLTE Carte</h4>";
                            echo "<a href='./ajouter-carte.php?nature=4GLTE' class='btn btn-success btn-md' style='float: right; margin-right: 10px;' title='ajouter carte 4G'>";
                                echo "<i class='fa fa-plus'></i>";
                            echo "</a>";
                        ?>
                        <hr>
                        <div id="scrollContent">
                            <table class="table table-striped table-advance table-hover">
                                ...
                                <tbody id="scrollContent">
                                    <?php 
                                    foreach($data_4glte as $carteData)
                                    {
                                       echo "<tr>";
                                       echo "<th scope='row'>".$carteData["id"]."</th>";
                                       echo "<td>".$carteData["nom"]."</td>";
